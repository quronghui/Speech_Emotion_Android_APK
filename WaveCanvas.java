...
public class WaveCanvas {
	...
	public static ArrayList<Short> shorts = new ArrayList<Short>();//缓冲区数据
...
//	         canvas.drawLine(0, height*0.75f+20, sfv.getWidth(),height*0.75f+20, paintLine);//第3根线
    ...
    class WriteRunnable implements Runnable {
		...
		public void run() {
			try {
				...
				Log.d("@@","path："+savePcmPath);
				Log.d("@@","path："+saveWavPath);
    ...
    /*public static ShortBuffer getShortBuffer(){
		...
		short[] buffer = new short[shorts.size()];
		for(int i =0;i< shorts.size();i++){
			buffer[i] = shorts.get(i);
		}
